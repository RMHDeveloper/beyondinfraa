...
// GET — list all intake tokens (admin)
export async function GET() {
  await requireSession();
  const tokens = await db.buyerIntakeToken.findMany({
...
// POST — create a new intake token link
export async function POST(req: Request) {
  ...
  const token = await db.buyerIntakeToken.create({
    data: {
      categoryId,
      label: label ?? null,
